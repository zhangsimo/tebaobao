...
//字符串解析为自定义格式的消息
function stringToCustomMsg(msg) {
  // var customMsg = { "id": "", "name": "", "image": "", "cmd": -1000, "msg": "" };
  // var strs = msg.split(" ");
  // customMsg.id = strs[0];
  // customMsg.name = strs[1];
  // customMsg.image = strs[2];
  // customMsg.cmd = parseInt(strs[3]);
  // if (strs.length > 4) {
  //   customMsg.msg = strs[4];
  // } else {
  //   customMsg.msg = "";
  // }
...
var CUSTOM_MSG_CMD = {
  ...
  //自定义系统消息类型
  "SYSTEM_MSG_AUDIENCE_ENTER_ROOM": 101, //观众进入房间
  "SYSTEM_MSG_AUDIENCE_LEAVE_ROOM": 102, //观众离开房间
  "SYSTEM_MSG_HOST_BACK": 103, //主播回到直播
  "SYSTEM_MSG_HOST_LEFT": 104, //主播离开直播
  "SYSTEM_MSG_REFRESH_PRODUCT_LIST": 106, //刷新商品列表
  "SYSTEM_MSG_RECOMMEND_GOODS": 107
};
...
function mySendMsg(msgtosend) {
  ...
  // var customMsgString = customMsgToString(loginInfo.identifier, loginInfo.identifierNick, loginInfo.headurl, CUSTOM_MSG_CMD.MSG_NORMAL, msgtosend);
  // console.log('sss', customMsgString)
  ...
  // var custom_obj = new webim.Msg.Elem.Custom("{ 111 13226 1 伊诺 0 123 334 }", null, null);
  var obj = {
    data: {
      nick_name: loginInfo.identifierNick,
      headimg: loginInfo.userAvatar,
      show_message: msgtosend,
      is_anchor: false
    }
  }
  var strr = JSON.stringify(obj);
  var text_obj = new webim.Msg.Elem.Text(strr);
  msg.addText(text_obj);
...
//自定义格式的消息转化为字符串
function customMsgToString(id, name, image, cmd, msg) {
  if (msg == "") {
    return "{" + id + " " + name + " " + image + " " + cmd + "}";
  } else {
    return "{" + id + " " + name + " " + image + " " + cmd + " " + msg + "}";
  }
  // var obj = "TBBTXIMCustomElemFaceUrl:" +  image + ",TBBTXIMCustomElemNickName:" + name + ",TBBTXIMCustomElemNickText:" + msg;
...
module.exports = {
  ...
  stringToCustomMsg: stringToCustomMsg,
  CUSTOM_MSG_CMD: CUSTOM_MSG_CMD,
  mySendMsg: mySendMsg
};